module ActiveRecord
  module Associations
    class HasManyThroughWithMultipleForeignAssociation < HasManyAssociation #:nodoc:
        ...
        def construct_owner_attributes(reflection)
          if as = reflection.options[:as]
            if as.is_a?(Array)
              attributes = {}
              as.each do |as_as|
                attributes["#{as_as}_id"] = @owner.id
                attributes["#{as_as}_type"] = @owner.class.base_class.name.to_s
              end
            else
            { "#{as}_id" => @owner.id,
